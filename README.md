...
|---|---|---|---|
| Cheese | 0.98 | 0.94 | 0.96 |
| Pepperoni | 0.94 | 0.98 | 0.96 |
...
|---|---|---|
| **Actual Cheese**    | 94 | 6 |
| **Actual Pepperoni** | 2  | 98 |

Validation accuracy rose from 78.0% (epoch 1) to 96.0% (epoch 10), with a brief dip to 91.0% at epoch 7 before recovering — training and validation accuracy track closely throughout, with no signs of significant overfitting.

## Key Insight

A relatively shallow 3-layer CNN reaches 96% accuracy on this task within just 10 epochs, showing that visually distinct classes (plain vs. topped pizza) don't require deep architectures to separate reliably. This contrasts with the tabular-data projects in this portfolio (Total Spend, Churn prediction), where linear and ensemble models struggled to beat a naive baseline — here, a clear, learnable visual signal is present in the images themselves.

**Technical note:** a warning is raised during training (`sparse_categorical_crossentropy received from_logits=True, but the output was produced by a Softmax activation`), since the final layer already applies softmax while the loss function is configured to expect raw logits. Training and evaluation still complete correctly, but this is a minor inconsistency worth cleaning up in a future iteration.
